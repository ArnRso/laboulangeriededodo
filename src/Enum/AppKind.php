...
enum AppKind: string
{
    ...
    public function pitch(): string
    {
        return match ($this) {
            ...
            self::UBER => 'Une course terminée : trajet, chauffeur, voiture, note du chauffeur.',
            ...
            self::REVOLUT => 'Une transaction : bénéficiaire, référence, catégorie, reçu joint.',
            self::PAYPAL => 'Un paiement reçu : expéditeur, petit mot, pièce jointe.',
    ...
    public function headline(array $data): string
    {
        return match ($this) {
            ...
            self::SNAPCHAT => sprintf('%s t\'a envoyé un Snap 👻', self::text($data, 'sender', 'le.pot.agé')),
            self::X => sprintf('%s a posté', self::text($data, 'displayName', 'le.pot.agé')),
            self::BEREAL => '⚠️ Time to BeReal ⚠️',
            self::YOUTUBE => sprintf('%s a mis en ligne une vidéo', self::text($data, 'channel', 'le.pot.agé')),
            ...
            self::WHATSAPP, self::MESSENGER, self::IMESSAGE => sprintf('%s t\'a envoyé un message', self::text($data, 'contact', 'le.pot.agé')),
            ...
            self::UBER => sprintf('Ta course avec %s est terminée', self::text($data, 'driver', 'le.pot.agé')),
            ...
            self::REVOLUT => sprintf('%s · transaction', self::text($data, 'counterparty', 'le.pot.agé')),
            self::PAYPAL => sprintf('Tu as reçu un paiement de %s', self::text($data, 'counterparty', 'le.pot.agé')),
            self::LYDIA => sprintf('%s t\'a envoyé un virement 💸', self::text($data, 'counterparty', 'le.pot.agé')),
